...
| U13 | **Google ilə giriş açarları** (→ 0062) | console.cloud.google.com/auth → External + **In production** + loqo YÜKLƏMƏ (brend yoxlaması tetiklənir) · scope-lar yalnız `openid`/`email`/`profile` (non-sensitive → audit yoxdur) · redirect URI **hərfi-hərfinə** `https://app.agradex.com/api/auth/google/callback` → `GOOGLE_CLIENT_ID`+`GOOGLE_CLIENT_SECRET` `.env`-ə | **✅ KOD HAZIR (`645b2ed`, migration 0062 canlıda):** `routers/oauth.py` server-tərəfli auth-code axını, `users.google_sub` partial unique index, hesab birləşdirmə (subject → təsdiqli email), 8 dildə düymə + 6 xəta mesajı. Açarsız **dormant**: `/providers` → `google:false`, hər iki route **404**, düymə render olunmur. ⚠️ Qalıq: **məxfilik siyasəti 8 dildə** Google-u data emalçısı kimi əlavə etməlidir | **Yüksək** | ⏳ açar gözləyir |

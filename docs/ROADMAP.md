...
>
> **Son yenilənmə:** 2026-07-22 — **🎉 12/12 task canlıda** (T0-T14 seçilmiş dəst): T0 T1 T2 T4 T5 T6
> T8 T9 T10 T11 T12 ✅ · T13 🚀. Növbəti dalğa: istifadəçi-bloklu (§B U1-U12) + qalan aşağı-prioritet
> T15-T26. E+T tək backlog (§C). Gap-scan `wf_c498734d`, kod ilə yoxlanmış.
...
| U3 | **Email/OTP (Resend)** — Sprint B#1 | Resend hesabı + `RESEND_API_KEY` + CF SPF/DKIM | **✅ KOD HAZIR (f71d1b8):** 0024 email_verified/otp; notify.py Resend; signup OTP + /verify-otp + /resend-otp; OtpVerify.tsx. Açarsız auto-verify. | **Yüksək** | ⏳ açar gözləyir |
...
| U5 | **app.agradex.com** (renamed from panel) — Sprint B#2 | CF: `app` A → 95.216.208.82 | nginx bloku + cookie `.agradex.com` + redirect + middleware; bax `deploy/APP_ACTIVATION.md` | Orta | ⏳ |
...
| **T18** | Çoxdilli lokalizasiya (EN/TR/DE; RU spec §25) | §25 | i18n | M | ⚪ | yox | 🚀 (Phase 4: 4-locale infra + path-prefix + switcher CANLI; primary/nav/home/settings/landing/pricing-prose/auth ekranları tərcümə olunur; **qalıq**: prose mühərrikləri insights.ts/indexStatus.ts + metadataOptions + pricing feature-cədvəli + field-detail tabları — bax [[i18n-architecture]] Server-Component gotcha) |

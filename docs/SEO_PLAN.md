...
> **Status: TƏSDİQLƏNDİ, S0 TAM CANLI (2026-08-02).** Qərarlar: **Q1** = ictimai müəllif — **Sabir Ismayilbayli** (sahibin işçisi, LinkedIn `in/sabirismayilbayli`; `app/src/lib/author.ts`) · **Q2** = məqalələri Claude yazır, sahib publish-dən əvvəl təsdiqləyir; infra = mövcud `/guide` naxışı · **Q3** = ES locale hədəfdən sonra · **Q4** = auto-redirect qalır + bot-istisna · **Q5** = GSC + GoAccess (`deploy/goaccess-report.sh`, server-log, cookie-siz).
> **DALĞA 2 ✅ CANLI (2026-08-02 axşam, sahibin birbaşa sifarişi ilə plan genişləndi):** `/blog` infrastrukturu (Article JSON-LD, byline, registry-əsaslı sitemap, mövcud-variant-məhdud hreflang) + **35 məqalə = 7 mövzu × 5 dil (az/en/ru/de/es)** — hər dil bazar adaptasiyası (KZ buğdası/UZ pambığı ru-da, Beregnung de-də, lote/espigazón es-də), eyni ilk-əl data nöqtələri (mod-5 keçid ritmi, NDMI −0.13, FAO-56 −44 mm) + **es locale canlı** (marketinq-öncə qismən, fallback es→en→az; Q3 qərarını sahib qabaqladı). Sitemap 152→**215 URL**. ⚠️ Q2 axını üzrə məqalələr sahibin redaksiya baxışını gözləyir — canlıdadır, amma sahib oxuyub düzəliş istəyə bilər.
> **T1 ✅ (2026-08-02, sahibin Chrome-u ilə):** GSC Domain property `agradex.com` **doğrulandı** (CF DNS TXT `google-site-verification=YsChr…dLeM` — silmə!), sitemap submit edildi, `/` + `/demo` + `/how-it-works` üçün indeksləmə tələb olundu. **Mühüm tapıntı:** Google saytı 2026-07-27-də artıq crawl etmişdi və **"Duplicate without user-selected canonical"** verdikti ilə indeksdən kənarda saxlamışdı — K2/K4 diaqnozunun birbaşa təsdiqi; fix-lər məhz bunu həll edir. **T5 ✅:** CF "Always Use HTTPS" ON + `www→apex` 301 Redirect Rule (template) — canlı curl: hər 4 host variantı kanonikə 301 verir. **Qalan:** Bing Webmaster (OAuth tələb etdiyi üçün ayrıca icazə lazımdır), T15 PSI.

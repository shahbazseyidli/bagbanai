...
**D0 ✅ (98e43cb)** · **D1 ✅ (0f57db2)** · **D2 🔨 (013802a — 1-ci dilim canlı)** · D3 ⬜ · D4 ⬜ · D5 ⬜ — hər addım (D0.1…D5.5) bitəndə commit hash + ✅ ilə işarələ. Bu sənəd `ROADMAP.md` §C ilə paralel işləyir (T-backlog funksional, bu — dizayn/İA).

**D2 detal (2026-07-22) — 1-ci dilim (təhlükəsiz, contained):** D2.3-lite ✅ (sahə tabları **9→3 niyyət qrupu** VƏZİYYƏT/İŞLƏR/MƏLUMAT + ikinci-səviyyə chip sırası; URL `?tab=` saxlanıldı) · D2.7 ✅ (migration 0025 `fields.deleted_at`; DELETE→soft + `/restore`; get 404/list/count filtrləri; 6s undo bar — canlı test: sil→404→restore→200). **D2 qalan (2-ci dilim, `?ui=v2` altında, brauzer-test tələb edir):** D2.1 bottom nav + desktop sidebar · D2.2 "Bu gün" kart ana ekranı · D2.3 tam-ekran xəritə + sürüşən 3-snap sheet · D2.4 Ətraflı analiz sensor-birləşmə · D2.5 bildiriş mərkəzi deep-link · D2.6 kamera FAB. Bunlar yeni marşrutlar (/fields, /more) + drag fizikası tələb edir — canlı prod-da ehtiyatla, ayrıca.

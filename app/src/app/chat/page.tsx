      ...
      <div className="grid gap-0 overflow-hidden rounded-xl border-[1.5px] border-slate-300 bg-white md:grid-cols-[300px_1fr]" style={{ minHeight: "60vh" }}>
        ...
        <div className={`border-slate-200 md:border-r ${selected ? "hidden md:block" : ""}`}>
          {convs === null ? <div className="p-4"><Spinner /></div> : convs.length === 0 ? (
            <div className="p-6 text-center text-sm text-slate-500">{t("app.chat.emptyConversations")}</div>
          ) : convs.map((c) => (
        ...
        <div className={`flex flex-col ${selected ? "" : "hidden md:flex"}`}>
          {!selected ? (
            <div className="flex flex-1 items-center justify-center p-6 text-sm text-slate-400">{t("app.chat.selectConversation")}</div>
          ) : (
            <>
              ...
              <div ref={bodyRef} className="flex flex-1 flex-col gap-2 overflow-y-auto p-4" style={{ maxHeight: "50vh" }}>
                ...
                {msgs.length === 0 && <div className="text-center text-xs text-slate-400">{t("app.chat.writeFirstMessage")}</div>}
              </div>
              <div className="flex gap-2 border-t border-slate-200 p-3">
                <input className="input flex-1" value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") send(); }} placeholder={t("app.chat.messagePlaceholder")} />

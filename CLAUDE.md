...
- **DB:** Postgres 16 + PostGIS (Docker). Sıralı miqrasiyalar `db/migrations/0001..0025` (`public.schema_migrations`, `db/migrate.sh`). **0014** = knowledge layer. **0015→0023** = v2.1 tasks (partial-status/alert_state/field_gdd/index_baseline/photo_diagnoses/water_balance/benchmark-hardening/pest_risk/fertilizer). **0024** = email_verified+otp (U3) + messaging_channels/message_log (U4). **0025** = `fields.deleted_at` (soft-delete, D2.7). Seed `db/seeds` (subsidiya 117 + crop_thresholds index_norms + inline seeds pest_risk_models/crop_nutrient_norms — deploy-da `load_seeds.py` MÜTLƏQ işə düşməlidir).
